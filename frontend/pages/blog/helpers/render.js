...
window.displayPosts = (posts) => {
    ...
    if (!posts?.length) {
        grid.innerHTML = '<p style="color:#FFD700;text-align:center;font-size:1.2rem;padding:3rem;">No news available.</p>';
        return;
    }
    ...
    grid.innerHTML = posts.map(post => {
        const title   = (lang === 'en' ? (post.title_en || post.title_sr || post.title) : (post.title_sr || post.title || post.title_en)) || 'Untitled';
        const content = (lang === 'en' ? (post.content_en || post.content_sr || post.content) : (post.content_sr || post.content || post.content_en)) || '';
        const short   = content.length > 150 ? content.substring(0, 147) + '...' : content;

        let imagePath = '';
        if (post.image) {
            imagePath = (post.image.startsWith('http') || post.image.startsWith('/'))
                ? post.image
                : '/' + post.image.replace(/^\.\.\//, '');
        }

        const date      = new Date(post.created_at.replace(' ', 'T'))
            .toLocaleDateString(lang === 'en' ? 'en-US' : 'sr-RS', { year:'numeric', month:'long', day:'numeric' });
        const featured  = post.featured == 1 ? `<span class="news-badge">${featuredTxt}</span>` : '';
...
window.setupBlogPagination = (pagination, onPageChange) => {
    ...
    div.querySelectorAll('.page-link').forEach(link => {
        link.addEventListener('click', (e) => { e.preventDefault(); onPageChange(parseInt(link.dataset.page)); });
    });
};
