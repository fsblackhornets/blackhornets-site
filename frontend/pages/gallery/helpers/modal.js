window.showGalleryDetail = (image, imagePath) => {
    ...
    const formatted = new Date(image.created_at).toLocaleDateString(locale, { year: 'numeric', month: 'long', day: 'numeric' });
    document.getElementById('galleryModalDate').innerHTML =
        `<i class="fas fa-calendar-alt"></i> <span>${dateLabel}: ${formatted}</span>`;

    document.getElementById('galleryModalDescription').textContent = window.getImageDescription(image);
