...
window.setupGalleryNavigation = () => {
    ...
    if (modal) {
        modal.querySelector('.close-modal')?.addEventListener('click', window.closeGalleryModal);
        modal.addEventListener('click', (e) => { if (e.target === modal) window.closeGalleryModal(); });
    }

    if (prevBtn) prevBtn.addEventListener('click', (e) => { e.stopPropagation(); window.navigateGallery(-1); });
    if (nextBtn) nextBtn.addEventListener('click', (e) => { e.stopPropagation(); window.navigateGallery(1); });
