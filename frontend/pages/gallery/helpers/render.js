...
window.openViewAll = (gridId) => {
	...
	function renderModal() {
		const slice = images.slice(modalPage * MODAL_PER_PAGE, (modalPage + 1) * MODAL_PER_PAGE);
		grid.innerHTML = '';
		slice.forEach((img, i) => {
		...
		pageLabel.textContent = totalModalPages > 1 ? `${modalPage + 1} / ${totalModalPages}` : '';
		document.getElementById('viewAllPrev').style.opacity = modalPage === 0 ? '0.3' : '1';
		document.getElementById('viewAllNext').style.opacity = modalPage >= totalModalPages - 1 ? '0.3' : '1';
		document.getElementById('viewAllNav').style.display = totalModalPages > 1 ? 'flex' : 'none';
	}

	document.getElementById('viewAllPrev').onclick = () => { if (modalPage > 0) { modalPage--; renderModal(); } };
	document.getElementById('viewAllNext').onclick = () => { if (modalPage < totalModalPages - 1) { modalPage++; renderModal(); } };
