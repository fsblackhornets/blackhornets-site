...
document.addEventListener("DOMContentLoaded", function () {
	...
	async function fetchTeamMembersCount() {
		try {
			...
			if (data.success && data.members) {
				...
				const teamMembersCountEl =
					document.getElementById("team-members-count");
	...
	document
		.querySelectorAll(
			".story-content, .mission-box, .vision-box, .department-card, .stat-item",
		)
		.forEach((el) => {
			el.style.opacity = "0";
	...
	function animateNumber(element, start, end, duration) {
		if (isNaN(end) || end === start) {
			element.textContent = isNaN(end) ? start : end;
			return;
		}
		let current = start;
