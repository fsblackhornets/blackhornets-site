...
window.buildCardElement = (
	person,
	{ positionLabel, positionClass = "", team = "", departmentLabel = "", t },
) => {
	const defaultImage = "/frontend/assets/images/W logo.png";
	const profileImage =
		person.profile_picture && person.profile_picture !== "default.jpg"
			? `/uploads/profiles/${person.profile_picture}`
			: defaultImage;

	const teamClass = window.TEAM_COLOR_CLASS[team] || "";
	const card = document.createElement("div");
	card.className = ["member-card", positionClass, teamClass]
		.filter(Boolean)
		.join(" ");
...
window.createLeaderCard = (leader, position, department = "") => {
	...
	const positionLabel =
		position === "Team Leader"
			? t.teamLeader || position
			: position === "Project Leader"
				? t.projectLeader || position
				: position === "Sub Leader"
					? t.subLeader || position
					: position;

	const departmentLabel =
		department === "Mechanical Engineering"
			? t.mechanicalEngineering || department
			: department === "Electrical Engineering"
				? t.electricalEngineering || department
				: department === "Business Team"
					? t.businessTeam || department
					: department;
...
window.createMemberCard = (member) => {
	...
	const departmentLabel =
		deptMapping[deptName] || window.decodeHtmlEntities(deptName);

	const team = member.team === "bot" ? "operating_business" : member.team || "";

	return window.buildCardElement(member, {
		positionLabel,
		positionClass,
		team,
		departmentLabel,
