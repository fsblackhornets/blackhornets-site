...
window.updateLeadershipStructure = (data) => {
	...
	if (data.team_leader) {
		hierarchyBox.appendChild(
			window.createLeaderCard(data.team_leader, "Team Leader"),
		);
	}
	...
	const sectors = [
		{
			...
			pl: data.business_project_leader || placeholderBusinessPL,
			label: "Business Team",
			cssClass: "operating-business",
			structure: teamStructure.operating_business,
		},
		{
			key: "mechanical",
			pl: data.mechanical_project_leader,
			label: "Mechanical Engineering",
			cssClass: "mechanical",
			structure: teamStructure.mechanical,
		},
		{
			key: "electrical",
			pl: data.electrical_project_leader,
			label: "Electrical Engineering",
			cssClass: "electrical",
			structure: teamStructure.electrical,
		},
	];
	...
	sectors.forEach((sector) => {
		...
		if (sector.pl) {
			column.appendChild(
				window.createLeaderCard(sector.pl, "Project Leader", sector.label),
			);
		}
		...
		sector.structure.departments.forEach((subteamName) => {
			...
			box.addEventListener("click", () =>
				window.showDepartmentMembers(sector.key, subteamName),
			);
		...
		btn.addEventListener("click", () =>
			window.showDepartmentMembers(sector.key),
		);
