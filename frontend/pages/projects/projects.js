...
const PROJECT_HOOKS = [
    '/frontend/pages/projects/hooks/fetch.js',
];

const loadProjectScripts = (srcs) => Promise.all(
...
document.addEventListener('DOMContentLoaded', () => {
    Promise.all([loadProjectScripts(PROJECT_HELPERS), loadProjectScripts(PROJECT_HOOKS)]).then(() => {
        ...
        document.querySelectorAll('.coming-soon .view-project').forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                window.showNotification?.('This project is currently under development. Stay tuned!');
            });
        });
