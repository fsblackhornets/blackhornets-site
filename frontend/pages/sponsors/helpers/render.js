...
window.groupSponsorsByTier = (sponsors) => {
	const tiers = {};
	(window.SPONSOR_TIERS || []).forEach((t) => {
		tiers[t] = [];
	});
...
window.displaySponsors = (sponsorsByTier, container) => {
	...
	Object.keys(sponsorsByTier).forEach((tier) => {
		...
		sponsors.forEach((sponsor) => {
			...
			const desc =
				lang === "en" && sponsor.description_en
					? sponsor.description_en
					: sponsor.description || "";
			const descHtml = desc ? `<p class="sponsor-description">${desc}</p>` : "";
			const websiteHtml = sponsor.website
				? `<a href="${sponsor.website}" target="_blank" rel="noopener" class="sponsor-website" onclick="event.stopPropagation()"><i class="fas fa-external-link-alt"></i> ${t.visitWebsite || "Visit Website"}</a>`
				: "";
			const hoverHtml =
				desc || sponsor.website
					? `<div class="sponsor-hover-info"><p class="sponsor-hover-name">${sponsor.name}</p>${descHtml}${websiteHtml}</div>`
					: "";
