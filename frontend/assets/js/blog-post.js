    ...
    fetch(`../api/posts/read.php?id=${postId}`)
        ...
        .then(data => {
            if (data.status !== 'success' || !data.data) {
                document.querySelector('.single-post .post-wrapper').innerHTML = '<p style="color:red;text-align:center;">Post not found.</p>';
                return;
            }
            renderPost(data.data);
        })
        .catch(() => {
            document.querySelector('.single-post .post-wrapper').innerHTML = '<p style="color:red;text-align:center;">Error loading post.</p>';
        });
}
...
function renderPost(post) {
    ...
    const content = (currentLang === 'en' ? (post.content_en || post.content_sr || post.content) : (post.content_sr || post.content || post.content_en)) || '';

    // Render categories
    let categories = '';
    if (post.featured == 1) categories += `<span class="category-tag">${escapeHtml(featuredText)}</span>`;
    if (post.category) categories += `<span class="category-tag">${escapeHtml(post.category)}</span>`;
    ...
    if (post.created_at) {
        ...
        formattedDate = dateObj.toLocaleDateString(currentLang === 'en' ? 'en-US' : 'sr-RS', options);
    }
    // Image
    let imagePath = '';
    if (post.image) {
        imagePath = post.image.startsWith('/') ? post.image : '/' + post.image.replace(/^\.\.\//, '');
    }
    // Author
    ...
    `;
    document.querySelector('.single-post .post-featured-image img').src = imagePath || 'https://picsum.photos/1200/600?random=20';
...
function renderRelatedPosts(currentId) {
    window.API.posts.getAll()
        .then(data => {
            ...
            if (relatedTitle) {
                relatedTitle.textContent = currentLang === 'en' ? 'Related Posts' : 'Povezane objave';
            }

            if (posts.length === 0) {
                container.style.animation = 'none';
                container.innerHTML = '<p class="related-no-posts">' + (currentLang === 'en' ? 'No other posts found.' : 'Nema drugih objava.') + '</p>';
                return;
            }
            ...
            function buildCard(post) {
                ...
                const shortContent = plainContent.length > 120 ? plainContent.substring(0, 117) + '...' : plainContent;
                let formattedDate = '';
                if (post.created_at) {
                    const dateObj = new Date(post.created_at.replace(' ', 'T'));
                    formattedDate = dateObj.toLocaleDateString(currentLang === 'en' ? 'en-US' : 'sr-RS', { year: 'numeric', month: 'short', day: 'numeric' });
                }
                ...
                return `
                    <a href="blog-post.html?id=${parseInt(post.id)}" class="related-card">
                        <div class="related-card-image">
                            ${imagePath
                                ? '<img src="' + escapeHtml(imagePath) + '" alt="' + escapeHtml(title) + '" onerror="this.parentElement.innerHTML=\'<div class=\\\'related-no-image\\\'><i class=\\\'fas fa-image\\\'></i></div>\'">'
            ...
            let cardsHtml = '';
            displayPosts.forEach(post => { cardsHtml += buildCard(post); });
