...
const TEAM_HOOKS = ["/frontend/pages/team/hooks/fetch.js"];

const loadTeamScripts = (srcs) =>
	Promise.all(
		srcs.map(
			(src) =>
				new Promise((resolve) => {
...
document.addEventListener("DOMContentLoaded", () => {
	Promise.all([
		loadTeamScripts(TEAM_HELPERS),
		loadTeamScripts(TEAM_HOOKS),
	]).then(() => {
		// Shared state
		...
		document.querySelector(".back-btn")?.addEventListener("click", () => {
			...
			document
				.querySelector(".leadership-hierarchy")
			...
			if ((window.allTeamMembers || []).length > 0) {
				window.fetchTeamMembers().then(() => {
					if (window.currentViewTeam) {
						const translatedDept = window.getTranslatedDeptName?.(
							window.currentViewDepartmentEnglish,
						);
						window.showDepartmentMembers?.(
							window.currentViewTeam,
							translatedDept,
							true,
						);
					}
