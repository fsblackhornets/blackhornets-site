...
const ROLE_ORDER = ["team_leader", "project_leader", "sub_leader", "team_member"];
...
export async function GET() {
	try {
		...
		const members = rows.map((r) => ({
			...
			department_name: r.department ? (DEPARTMENT_NAMES[r.department] ?? r.department) : null,
		}));
		...
		return NextResponse.json({
			...
			mechanical_project_leader: members.find((m) => m.role === "project_leader" && m.team === "mechanical") ?? null,
			electrical_project_leader: members.find((m) => m.role === "project_leader" && m.team === "electrical") ?? null,
			business_project_leader: members.find((m) => m.role === "project_leader" && m.team === "operating_business") ?? null,
		});
	} catch {
		return NextResponse.json(
