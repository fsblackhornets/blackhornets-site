...
test("contact form submission appears in admin messages panel", async ({ page }) => {
	...
	await page.fill("textarea[name=message]", "Automated E2E message for admin panel check.");
	await page.getByRole("button", { name: /send/i }).click();
	await expect(page.getByRole("main").getByText(/message sent/i)).toBeVisible({ timeout: 10000 });
