...
test("/blog/[id] renders post title on detail page", async ({ page }) => {
	...
	const postLink = page.getByRole("link", { name: new RegExp(titleEn, "i") }).first();
	await expect(postLink).toBeVisible({ timeout: 8000 });
	await postLink.click();

	// Detail page renders title in h1
	await expect(page.locator("h1")).toContainText(titleEn, { ignoreCase: true, timeout: 8000 });
});
...
test("/projects/[id] renders project name on detail page", async ({ page }) => {
	...
	await submitAndApprove(
		...
		async (p) => {
			...
			await p.fill("textarea[name=description], input[name=description]", "Detail page E2E project.").catch(() => {});
	...
	const projectLink = page.getByRole("link", { name: new RegExp(name, "i") }).first();
	await expect(projectLink).toBeVisible({ timeout: 8000 });
	await projectLink.click();

	// Detail page renders project name in h1
	await expect(page.locator("h1")).toContainText(name, { ignoreCase: true, timeout: 8000 });
});
